...
def fetch_data_for_last_4_days():
    data_frames = []
    current_date = datetime.now()
    days_collected = 0
    while days_collected < 4:
        try:
            df = fetch_data_for_date(current_date)
            if not df.empty:
                data_frames.append(df)
                days_collected += 1
        except Exception as e:
            print(f"No data available for {current_date.strftime('%d-%m-%Y')}. Skipping...")
        current_date -= timedelta(days=1)
    return data_frames

# Fetch data for the last 4 trading days
data_frames = fetch_data_for_last_4_days()

# Create separate dataframes for each day
df = data_frames[0] if len(data_frames) >= 1 else pd.DataFrame()
df1 = data_frames[1] if len(data_frames) >= 2 else pd.DataFrame()
df2 = data_frames[2] if len(data_frames) >= 3 else pd.DataFrame()
df3 = data_frames[3] if len(data_frames) >= 4 else pd.DataFrame()
...
def filter_data_for_common_symbols(df_list):
    ...
    # Reset index for each filtered DataFrame
    for i in range(len(filtered_dfs)):
        filtered_dfs[i].reset_index(drop=True, inplace=True)
    
    return filtered_dfs

df, df1, df2, df3 = filter_data_for_common_symbols([df, df1, df2, df3])

# Display the DataFrame for each day
#print("Data for Day 1:")
#print(df)
#print("\nData for Day 2:")
#print(df1)
#print("\nData for Day 3:")
...
def detect_gmarubozu(df):
    ...
    for index, row in df.iterrows():
        if abs(row['CLOSE']-row['OPEN']) > 0.03*(row['PREVCLOSE']) and\
           row['CLOSE'] > row['OPEN'] and\
...
# Save the merged DataFrame to Excel with the updated file name
merged_df.to_excel("output_all_bullish_candlestick_patterns.xlsx", index=False)
